...
{
    public class StaticLoadConfigurationRepository : IStaticLoadConfigurationRepository
    {
        private readonly ApplicationDbContext _context;
        public StaticLoadConfigurationRepository(ApplicationDbContext context)
        {
            _context = context;
        ...
        {      
            await _context.Database.ExecuteSqlRawAsync("DELETE FROM [StaticLoadTests]");
        }
        public Task InsertAsync(StaticLoadTest entry)
        {
            throw new NotImplementedException();
        }
        public async Task<StaticLoadTest> LoadConfiguration()
        {
            
            var configs = await _context.StaticLoadTests.ToArrayAsync();
            return configs[0];
        }
        public async Task UpdateConfiguration(StaticLoadTest config)
        {
          
                var pre = await (from p in _context.StaticLoadTests
